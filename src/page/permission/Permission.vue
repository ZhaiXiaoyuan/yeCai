            ...
            <div class="container-bd">
                <el-tabs v-model="activeName" @tab-click="handleClick">
                    ...
                    <el-tab-pane label="会计管理员" name="second">
                        ...
                        <el-table :data="accountantManagerList" border style="width: 100%;" ref="multipleTable">
                            <el-table-column prop="account" label="手机号" align="center"></el-table-column>
                            <el-table-column prop="name" label="姓名"  align="center"></el-table-column>
                            <el-table-column label="状态"  align="center">
                                <template slot-scope="scope">
                ...
                <el-dialog :title="(handleType=='add'?'新建':'编辑')+(activeName=='first'?'市场管理员':'会计管理员')" class="edit-dialog" :visible.sync="dialogFormVisible" width="40%">
                    <el-row type="flex">
        ...
        methods: {
            ...
            getMarketManagerList:function (pageIndex) {
                ...
                Vue.api.getMarketManagerList(params).then((resp)=>{
                    if(resp.respCode=='00'){
                        ...
                        this.marketManagerList=data;
                        //临时测试，未返回分页对象
                       /* this.marketManagerPager.total=data.userCount;*/
                        this.marketManagerPager.total=data.length;
                    }
                });
            },
            getAccountantManagerList:function (pageIndex) {
                ...
                Vue.api.getAccountantManagerList(params).then((resp)=>{
                    if(resp.respCode=='00'){
                        let data=JSON.parse(resp.respMsg);
                        this.accountantManagerList=data;
                        //临时测试，未返回分页对象
                        /* this.marketManagerPager.total=data.userCount;*/
                        this.accountantManagerPager.total=data.length;
                    }
                });
            },
            del:function (entry) {
